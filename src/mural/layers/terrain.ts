...
import { GROUND_TINT, MURAL_PALETTE, SEAM_FEATHER_WIDTH, Y_BANDS } from '../mural-vocabulary.js';
  ...
  return eras
    .map((era, index) => {
      ...
      return (
        ...
        `<rect x="${formatSvgNumber(startX - era.x)}" y="${formatSvgNumber(y)}" width="${formatSvgNumber(endX - startX)}" height="${formatSvgNumber(height)}" fill="${GROUND_TINT[era.tier]}"/>` +
        `</g>`
      );
    })
    .join('');
