...
export function renderTerrain(width: number, eras: PlacedEra[], world: World): string {
  return renderDistantBand(width, world) + renderEraGround(0, width, eras, world);
}
 ...
 * seams never gap. The edge eras reach leftX/rightX so the whole band covers those bounds.
 */
export function renderEraGround(leftX: number, rightX: number, eras: PlacedEra[], world: World): string {
  const y = Y_BANDS.horizonBottom;
  const height = Y_BANDS.roadBaseline - Y_BANDS.horizonBottom;
  return eras
    .map((era, index) => {
      const startX = index === 0 ? leftX : era.x;
      const endX = index === eras.length - 1 ? rightX : era.x + era.width + SEAM_FEATHER_WIDTH;
      return (
