# Stage 0 art-style prototypes

Three hand-built candidate styles for the epic's look. Each SVG cycles title card → chapter scene → ambient frame via SMIL. Embeds below use SHA-pinned raw URLs so github.com renders them through camo — the same pipeline a profile README uses. If they animate here, they animate in a profile.

Working pick: **universe-from-first-commit**. Override by naming another candidate in the decision PR's review.
